...
async function runValidation() {
  ...
  try {
    ...
    // 4. Request a Loan (AIS Credit Line)
    console.log('\n--- Step 3: Requesting Reputation-Backed Loan ---');
    // Fetch newly registered agent details first to ensure state is synchronized
    const agentRes = await axios.get(`${BACKEND_URL}/v1/agent/${agentAddress}`, HEADERS);
    const ais = agentRes.data.current_ais || 600;
    const ceiling = (ais / 1000.0) * 5000.0;
    console.log(`Agent AIS: ${ais} | Credit Ceiling: ${ceiling} ITK`);
